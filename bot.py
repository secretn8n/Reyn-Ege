...
# ── Flask (keep-alive для Render free tier) ──────────────────────────────────
...
def healthcheck():
    return {"status": "running", "tasks": len(ALL_TASKS), "variants": len(VARIANTS_META)}, 200
...
# Нормализуем все задания в плоский список
ALL_TASKS: list[dict] = []
...
for v in RAW:
    num = v["variant"]
    VARIANTS_META[num] = {
        # url намеренно не сохраняем — ссылки на внешний сайт не показываются
    ...
    def add(task, variant, text_key=None):
        ALL_TASKS.append({
            "id":        f"v{variant}_t{task['number']}_{random.randint(0,9999)}",
            "variant":   variant,
            "number":    task["number"],
...
# Пересоздаём id детерминировано после загрузки всех заданий
for i, task in enumerate(ALL_TASKS):
    task["id"] = f"v{task['variant']}_t{task['number']}_{i}"
...
def get_state(uid: int) -> dict:
    if uid not in USER_STATE:
        USER_STATE[uid] = {
            ...
            "queue":      [],
            "current_id": None,
            "stats":      {},       # task_id -> {"attempts": n, "correct": n}
            "wrong_ids":  set(),
            "filter_num": None,
            "filter_var": None,
            "answered":   False,
...
def escape(text: str) -> str:
    """Минимальный escape для MarkdownV2."""
    for ch in r"\_*[]()~`>#+-=|{}.!":
        text = text.replace(ch, f"\\{ch}")
    return text

def build_task_msg(task: dict) -> tuple[str, str]:
    """Возвращает (text, parse_mode). Без внешних ссылок."""
    v = task["variant"]
    n = task["number"]

    lines = [f"📋 *Задание {n}* — Вариант {v}"]

    # текст к заданиям 1-3 или 23-26
    if task["text_key"]:
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── ПО НОМЕРУ ──
        ...
        await q.edit_message_text(
            f"📋 Задание №{n}: найдено {len(tasks)} заданий из разных вариантов\\. Начинаем\\!",
            parse_mode=ParseMode.MARKDOWN_V2
        )
    ...
    # ── ПО ВАРИАНТУ ──
        ...
        s["queue"] = [t["id"] for t in tasks]
        await q.edit_message_text(
            f"📓 Вариант {v}: {len(tasks)} заданий\\. Начинаем\\!",
            parse_mode=ParseMode.MARKDOWN_V2
        )
        await send_next_from_queue(update, context)
    ...
    # ── ПОВТОРЕНИЕ ОШИБОК ──
        ...
        s["queue"] = wrong[:]
        await q.edit_message_text(
            f"🔁 Повторяем {len(wrong)} заданий с ошибками\\!",
            parse_mode=ParseMode.MARKDOWN_V2
        )
        await send_next_from_queue(update, context)
    ...
    # ── ОТМЕТИТЬ ПРАВИЛЬНО/НЕПРАВИЛЬНО ──
    elif data in ("mark_correct", "mark_wrong"):
        tid = s.get("current_id")
        if not tid:
            return
        if tid not in s["stats"]:
            s["stats"][tid] = {"attempts": 0, "correct": 0}
...
async def unknown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Используй кнопки или команду /start",
        reply_markup=main_menu_kb()
    )
...
def main():
    ...
    # Запускаем Flask в отдельном потоке (Render требует открытого HTTP-порта)
