...
ALL_TASKS: list[dict] = []        # каждый элемент — одно задание
VARIANTS_META: dict[int, dict] = {}  # variant_num -> {url, text_1_3, text_23_26}
...
# uid -> { mode, queue, current_id, stats, wrong_ids, filter_num, filter_var }
...
def get_state(uid: int) -> dict:
    if uid not in USER_STATE:
        USER_STATE[uid] = {
            ...
            "wrong_ids": set(),   # задания с ошибками для повторения
            "filter_num": None,
            "filter_var": None,
            "answered": False,    # уже нажал «показать ответ»
        }
...
def build_task_msg(task: dict) -> tuple[str, str]:
    ...
    if task["text_key"]:
        ...
        if ctx:
            # обрезаем до 800 символов с многоточием
